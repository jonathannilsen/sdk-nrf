...
#include "sfts.h"
	...
	/* TODO can this be enforced in Kconfig? */
	#if CONFIG_DFU_THREAD_PRIO <= CONFIG_UART_DFU_LIBRARY_THREAD_PRIO
	#error "DFU thread must have strictly lower priority than UART DFU"
	#endif
#endif
...
static uint8_t id_buf[IDENTIFY_BUF_SIZE];
static uint16_t id_buf_len;
...
static struct {
	...
	uint8_t rbuf_internal[CONFIG_DFU_THREAD_BUF_SIZE];
...
static int image_identify(const uint8_t *data, uint16_t len)
{
	int img_type;
...
static int next_image_segment_in(const uint8_t *data, uint16_t len)
{
#if CONFIG_DFU_THREAD
	/* Pipe data to DFU thread. */
	uint32_t put_len = ring_buf_put(&sfts_dfu_ctx.rbuf, data, len);
	if (put_len < len) {
...
static int first_image_segment_in(const uint8_t *data, uint16_t len)
{
...
static int on_sfts_data(struct bt_conn *conn, const uint8_t *data, uint16_t len)
{
	__ASSERT_NO_MSG(sfts_dfu_ctx.conn == conn);
...
static int on_sfts_complete(struct bt_conn *conn, const uint32_t crc)
{
	__ASSERT_NO_MSG(sfts_dfu_ctx.state != SFTS_DFU_STATE_IDLE);
	__ASSERT_NO_MSG(sfts_dfu_ctx.conn == conn);
